  ...
  return (
    <div className="bg-white rounded-[1.5rem] lg:rounded-[2.5rem] overflow-hidden shadow-xl border border-gray-100">
       <div className="p-5 lg:p-8 border-b border-gray-100 bg-gray-50/50">
          <h2 className="font-display font-black text-lg lg:text-xl text-kerala-dark">Pending Approvals ({orders.length})</h2>
          <p className="text-[10px] lg:text-[10px] uppercase font-bold text-gray-400 mt-1 tracking-widest">Verify Screenshot on WhatsApp before approving</p>
       </div>
       <div className="overflow-x-auto">
         <table className="w-full text-left min-w-[600px] lg:min-w-0">
            <thead className="bg-gray-50 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
               <tr>
                  <th className="px-6 lg:px-8 py-4">User Details</th>
                  <th className="px-6 lg:px-8 py-4">Tickets</th>
            ...
            <tbody className="divide-y divide-gray-50">
               {orders.length === 0 ? (
                 ...
                 <tr key={o.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 lg:px-8 py-4 lg:py-6 text-right space-x-2">
                       <div className="flex flex-col sm:flex-row gap-2 justify-end">
                        <button 
                          onClick={()=>handleApprove(o)} 
                          disabled={o.status === 'approved'}
                          className="text-[10px] bg-kerala-green text-white px-3 py-1.5 lg:px-4 lg:py-2 font-black uppercase border border-kerala-green rounded shadow-sm hover:scale-105 transition-all disabled:opacity-50 disabled:hover:scale-100"
                        >
                          {o.status === 'approved' ? 'OK' : 'Approve'}
                        </button>
                        <button onClick={()=>handleSendMessage(o)} className="text-[10px] bg-[#25D366] text-white px-3 py-1.5 lg:px-4 lg:py-2 font-black uppercase border border-[#25D366] rounded shadow-sm hover:scale-105 transition-all">
                          Chat
                        </button>
                      </div>
                    </td>
