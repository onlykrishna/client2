...
export default function Results() {
  const [selectedDay, setSelectedDay] = useState('2026-04-17');
  const draw = MOCK_DRAWS.find(d => d.id === selectedDay) || MOCK_DRAWS[0];

  const days = [
    { id: '2026-04-17', label: 'Today', date: 'Apr 17' },
    { id: '2026-04-16', label: 'Yesterday', date: 'Apr 16' },
    { id: '2026-04-15', label: 'Tuesday', date: 'Apr 15' },
  ];
  ...
  return (
    <div className="pb-20 bg-kerala-cream min-h-screen">
      <section className="bg-kerala-green pt-16 pb-32 px-4 text-white text-center">
        ...
        <p className="opacity-60 font-bold uppercase tracking-widest text-xs">Verified results released at 3:00 PM Daily</p>
      </section>

      <div className="container mx-auto max-w-4xl px-4 -mt-16">
        {/* Draw Selector */}
        <div className="flex bg-white p-2 rounded-3xl shadow-2xl mb-12">
           {days.map(day => (
             <button
               key={day.id}
               onClick={() => setSelectedDay(day.id)}
               className={`flex-1 py-4 px-2 rounded-2xl flex flex-col items-center gap-1 transition-all ${selectedDay === day.id ? 'bg-kerala-green text-white shadow-lg' : 'text-gray-400 font-bold'}`}
             >
                ...
                {day.id === '2026-04-17' && <span className="absolute top-2 right-2 bg-red-500 w-2 h-2 rounded-full animate-ping"></span>}
             </button>
           ))}
        </div>
          ...
          >
            {draw.status === 'upcoming' ? (
              <div className="bg-white rounded-[3rem] p-16 text-center shadow-xl border border-gray-100">
                 ...
                 <p className="text-gray-400 mb-8 max-w-md mx-auto">Today's winning numbers will be announced live at 3:00 PM. Stay tuned and best of luck!</p>
              </div>
            ) : (
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {draw.results.slice(0, 3).map((res, idx) => (
                      <div key={res.tier} className={`rounded-[2.5rem] p-8 text-center shadow-xl relative overflow-hidden ${idx === 0 ? 'bg-kerala-gold text-kerala-dark md:scale-110 md:z-10' : 'bg-kerala-dark text-white'}`}>
                        ...
                        <p className="text-4xl font-display font-black italic mb-4">{res.amount}</p>
                        <div className="bg-white/10 rounded-2xl py-4 border border-white/10">
                           <p className="text-3xl font-mono font-black tracking-widest">{res.winningNumbers[0]}</p>
                        </div>
                      ...
                      {draw.results.map((prize) => (
                        <div key={prize.tier} className="py-8 flex flex-col md:flex-row md:items-center gap-6 group">
                           ...
                           <div className="flex-1 flex flex-wrap gap-2">
                              {prize.winningNumbers.slice(0, 12).map(num => (
                                <span key={num} className="font-mono bg-kerala-cream text-kerala-dark px-4 py-2 rounded-xl text-sm font-black border border-kerala-green/5 group-hover:bg-kerala-gold/10 group-hover:border-kerala-gold/20 transition-colors">
                                   {num}
                                </span>
                              ))}
                              {prize.winningNumbers.length > 12 && (
                                <span className="bg-gray-100 text-gray-400 px-4 py-2 rounded-xl text-xs font-black flex items-center">
                                  + {prize.winningNumbers.length - 12} More
                                </span>
                              )}
                           </div>
                        </div>
